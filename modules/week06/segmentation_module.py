...
class SegmentationModule(BaseImageProcessor):
    ...
    def render_theory(self):
        ...
        **세그멘테이션(Segmentation)**은 이미지의 각 픽셀을 특정 클래스나 객체에 할당하는 작업입니다.
        ...
        U-Net은 **"U"자 모양**처럼 생겨서 U-Net이라고 부릅니다. 마치 계곡을 내려갔다가 다시 올라오는 모양이에요.
        ...
        **비유**: 망원경으로 사진을 찍듯이, 큰 그림을 점점 작게 압축합니다.
        ...
        - **무엇을 하나요?** 이미지를 점점 작게 만들면서 중요한 특징만 추출
        - **왜 필요한가요?** "이건 고양이야", "이건 배경이야" 같은 큰 의미를 이해하기 위해
        - **예시**: 512×512 → 256×256 → 128×128 → 64×64로 축소

        💡 *이미지는 작아지지만, 이해하는 정보는 더 많아집니다!*

        ---

        #### 🎯 2단계: Bottleneck (병목) - "핵심 이해"

        **비유**: 긴 글을 한 줄로 요약하는 것처럼, 가장 압축된 상태입니다.

        - **무엇을 하나요?** 이미지의 핵심 정보만 담은 가장 작은 표현
        - **왜 필요한가요?** "이 이미지에는 고양이 2마리와 소파가 있어"처럼 전체 상황을 파악

        💡 *가장 작지만, 가장 많은 의미를 담고 있어요!*
        ...
        **비유**: 압축 파일을 다시 풀듯이, 작은 이미지를 원래 크기로 키웁니다.
        ...
        - **무엇을 하나요?** 압축된 정보를 다시 원래 크기로 확대
        - **왜 필요한가요?** "각 픽셀이 고양이인지 소파인지" 구분하기 위해
        - **예시**: 64×64 → 128×128 → 256×256 → 512×512로 확대

        💡 *이미지를 키우면서 "이 픽셀은 고양이 귀", "저 픽셀은 소파" 같은 세밀한 정보를 복원해요!*

        ---

        #### ⭐ 4단계: Skip Connections (지름길 연결) - "디테일 보존"

        **비유**: 산을 내려갔다 올라올 때, 내려가면서 본 표지판을 기억하는 것과 같아요.

        - **무엇을 하나요?** 압축 단계(↓)의 세밀한 정보를 복원 단계(↑)에 직접 전달
        - **왜 필요한가요?** 압축하면서 잃어버린 "고양이 수염", "털 질감" 같은 디테일을 되살리기 위해
        - **효과**: 경계선이 더 선명하고 정확한 세그멘테이션 가능

        💡 *이게 U-Net의 핵심 비법입니다! 내려갈 때 본 정보를 올라올 때 다시 사용해요!*

        ---

        ### 🎓 왜 U-Net이 좋은가요?

        1. **디테일 보존**: Skip Connection 덕분에 세밀한 경계선까지 정확하게 분할
        2. **적은 데이터로 학습**: 의료 이미지처럼 데이터가 적어도 잘 작동
        3. **빠른 처리**: 효율적인 구조로 실시간 처리 가능
        4. **다양한 응용**: 의료, 위성 이미지, 자율주행 등 폭넓게 사용

        ---

        ## 4. Segment Anything Model (SAM) - 쉬운 설명
        ...
        **비유**: SAM은 **"만능 가위"** 같아요!
        ...
        일반 가위는 종이만 자르거나 천만 자르는 등 특화되어 있지만, SAM은 처음 보는 물건이라도 척척 잘라낼 수 있어요.
        ...
        **비유**: 외국인이 처음 본 과일도 "이게 과일이구나"라고 아는 것처럼
        ...
        - **예시**:
          ...
          - 자동차만 학습했는데 자전거도 분할 가능
        - **왜 대단한가요?** 기존 모델은 학습한 것만 분할 가능했어요

        💡 *1,100만 개 이미지로 학습해서 거의 모든 것을 알아요!*
        ...
        **비유**: 리모컨으로 TV 채널을 바꾸듯이, 클릭과 박스로 AI를 조종
        ...
        3가지 조작 방법:
        - **Point (클릭)**: "여기 있는 것 잘라줘!" 👆
        - **Box (박스)**: "이 영역 안에 있는 거 잘라줘!" ⬜
        ...
        **3단계 작동 방식** (쉽게 설명)
        ...
        #### 1단계: 이미지 이해하기 (Image Encoder) 🔍

        **비유**: 사진을 꼼꼼히 관찰하는 탐정

        - 이미지의 모든 부분을 분석
        - "여기는 고양이", "저기는 소파", "배경은 벽" 같은 정보 파악
        - Vision Transformer 사용 (눈으로 보듯 이미지 스캔)

        ---

        #### 2단계: 명령 이해하기 (Prompt Encoder) 🎯

        **비유**: 사용자의 말을 알아듣는 통역사

        - 클릭(Point): "이 위치에 있는 물건"
        - 박스(Box): "이 영역 안의 물건"
        - 마스크(Mask): "이미 선택된 것을 개선"

        ---

        #### 3단계: 결과 만들기 (Mask Decoder) ✂️

        **비유**: 가위로 정확하게 오려내기

        - 1단계(이미지 정보) + 2단계(명령) 결합
        - 정확한 경계선 찾기
        - **특별 기능**: 3개 후보 제시 (다양한 해석 가능)

        💡 *"고양이만 자를까요? 고양이+소파? 전체?"처럼 여러 선택지를 줘요!*
        ...
        | 구분 | 기존 모델 | SAM |
        |------|----------|-----|
        | **학습** | "고양이", "개" 등 정해진 것만 | 처음 보는 것도 분할 ⭐ |
        | **사용법** | 자동으로만 작동 | 클릭/박스로 조작 가능 ⭐ |
        | **속도** | 느림 (몇 초~몇 분) | 빠름 (< 1초) ⭐ |
        | **정확도** | 특정 데이터에만 정확 | 거의 모든 것에 정확 ⭐ |
        ...
        #### 1. 📸 **증명사진 배경 제거**
        - **예시**: 클릭 한 번으로 배경을 흰색/파란색으로 변경
        - **사용자**: 사진관, 온라인 증명사진 서비스

        #### 2. 🏷️ **AI 학습 데이터 만들기**
        - **예시**: 수천 장 이미지에서 자동으로 자동차, 사람 분할
        - **사용자**: AI 개발자, 데이터 라벨러
        ...
        **SAM = 처음 보는 것도 척척 + 클릭으로 조작 + 번개처럼 빠른 = 만능 세그멘테이션 AI**
                ...
                try:
                    ...
                    if "not enough memory" in str(e):
                        ...
                        **해결 방법**:
                        ...
                        또는 **Interactive 세그멘테이션** 탭을 사용하세요 (메모리 효율적).
                        """)
                    else:
                        st.error(f"에러 발생: {e}")
    ...
    def _app_auto_labeling(self):
        ...
        **워크플로우**:
        ...
        st.warning("⚠️ 메모리 사용량이 많습니다. 작은 이미지를 사용하거나 리사이즈하세요.")
        ...
        if uploaded:
            ...
            if st.button("🤖 자동 마스크 생성"):
                try:
                    ...
                    if "not enough memory" in str(e):
                        st.error("❌ 메모리 부족! 이미지를 더 작게 리사이즈하세요.")
                    else:
                        st.error(f"에러: {e}")
                except Exception as e:
                    st.error(f"예상치 못한 에러: {e}")
        ...
        if uploaded:
            ...
            if st.button("🔢 객체 카운팅", type="primary"):
                try:
                    ...
                    if "not enough memory" in str(e):
                        st.error("❌ 메모리 부족! 이미지를 더 작게 리사이즈하거나 Interactive 탭을 사용하세요.")
                    else:
                        st.error(f"에러: {e}")
                except Exception as e:
                    st.error(f"예상치 못한 에러: {e}")
